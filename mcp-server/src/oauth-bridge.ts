...
export async function handleOAuthBridge(req: IncomingMessage, res: ServerResponse, url: URL): Promise<boolean> {
  ...
  if (url.pathname === "/oauth/.well-known/openid-configuration") { send(res, 200, { issuer: base, authorization_endpoint: `${base}/oauth/authorize`, token_endpoint: `${base}/oauth/token`, jwks_uri: `${issuer}.well-known/jwks.json`, client_id_metadata_document_supported: true, token_endpoint_auth_methods_supported: ["none"], code_challenge_methods_supported: ["S256"], authorization_response_iss_parameter_supported: true }); return true; }
  ...
  if (url.pathname === "/oauth/callback") { try { const p = unpack(url.searchParams.get("state") ?? ""); const out = new URL(p.redirect); out.searchParams.set("code", pack({ upstream: url.searchParams.get("code"), ...p })); if (p.state) out.searchParams.set("state", p.state); out.searchParams.set("iss", base); res.writeHead(302, { location: out.href }); res.end(); } catch { send(res, 400, { error: "Invalid OAuth callback." }); } return true; }
