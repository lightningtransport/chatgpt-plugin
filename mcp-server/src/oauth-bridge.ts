...
function discovery() { return { issuer: bridgeIssuer, authorization_endpoint: `${bridgeIssuer}/authorize`, token_endpoint: `${bridgeIssuer}/token`, jwks_uri: `${issuer}.well-known/jwks.json`, client_id_metadata_document_supported: true, token_endpoint_auth_methods_supported: ["none"], code_challenge_methods_supported: ["S256"], authorization_response_iss_parameter_supported: true }; }

export async function handleOAuthBridge(req: IncomingMessage, res: ServerResponse, url: URL): Promise<boolean> {
  const isDiscovery = url.pathname === "/.well-known/openid-configuration/oauth" || url.pathname === "/.well-known/oauth-authorization-server/oauth" || url.pathname === "/oauth/.well-known/openid-configuration" || url.pathname === "/oauth/.well-known/oauth-authorization-server";
  if (!isDiscovery && !url.pathname.startsWith("/oauth/")) return false;
  if (!clientId || !secret) { send(res, 503, { error: "OAuth bridge is not configured." }); return true; }
  // RFC 8414 puts .well-known before an issuer path. The /oauth/* aliases
  // remain for clients that use the older path convention.
  if (isDiscovery) { send(res, 200, discovery()); return true; }
